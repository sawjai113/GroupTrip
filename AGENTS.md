...
- **Risk-tiered pipeline (adopted 2026-09-02)**: pipeline depth matches RISK SURFACE, not chunk count. Tier 2 (schema/RLS/DTO/store-rebuild/persistence/novel-SDK work) = full pipeline with @review per commit — that's where silent data loss lives and the gate has caught real bugs twice (chunk 3 case-sensitivity, chunk 4 time-dropped at rebuild sites). Tier 1 (pattern-following UI on established data) = thin @architect scope → @design only if new visual surface → @coding → user visual checkpoint as the UI gate → orchestrator verification (tests/build/secrets) → commit; @review batched to milestone closeout. Milestone boundaries get one comprehensive @review closeout audit instead of per-chunk gates. Unit tests passing is NOT sufficient for Tier 2 — review reads the wiring, which is where these bugs live.
